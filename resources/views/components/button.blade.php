...
@endphp
<button {{ $attributes->merge(['class' => $buttonStyle->getClass() . ' ' . (isset($awesomeIcon) && !isset($wide) ? 'pl-10' : '') . ' ' . (isset($wide) ? 'w-full' : '') . ' items-center group whitespace-nowrap relative flex justify-center py-2 px-4 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-offset-2']) }} {!! isset($click) ? 'onClick="' . $click . '"' : '' !!} type="{{ $type }}">
